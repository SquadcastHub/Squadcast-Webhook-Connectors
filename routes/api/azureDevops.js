...
// Create Card API
router.post('/azureDevops',authV1.auth, async (request, response) => {
    //HANDLE BAD REQUEST
    ...
    let adoURL = process.env.adoURL;
    if(!adoURL.endsWith('/')){
      adoURL+='/';
    }
    let url = adoURL + request.header("service") + "/_apis/wit/workitems/$issue"
    await axios({
      method: 'post',
      url: url,
      headers: {
